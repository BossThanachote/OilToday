...
export default async function HomePage() {
  ...
  const oilPrices = await db.oilPrice.findMany({
    orderBy: { updatedAt: 'desc' },
  });

  return (
    // ปรับพื้นหลังให้ดูสบายตาขึ้นเล็กน้อย
    <main className="min-h-screen bg-slate-50 font-sans">
      <div className="max-w-7xl mx-auto p-4 md:p-10">
        ...
        {/* --- Content Area --- */}
        {oilPrices.length === 0 ? (
          // กรณีไม่มีข้อมูล แสดง UI สวยๆ บอกให้ไป scrape
          ...
          // ✨ กรณีมีข้อมูล ส่งให้ Client Component จัดการ UI ทั้งหมด (Sidebar + List)
          // ตัวโค้ดส่วนนี้จะเข้าไปอยู่ใน components/OilExplorer.tsx แทนครับ
          <OilExplorer initialData={oilPrices} />
        )}

      </div>
      {/* --- ข้อมูลเพิ่มเติมและเครดิต --- */}
